const { CombatBuffer } = require('../src/services/rpg/combatBuffer');
...
async function run() {
  ...
  const bufObserve = new CombatBuffer({ text: 'miro alrededor buscando enemigos', room: {}, participant: {}, inventory: {} });
  ...
  const bufAttack = new CombatBuffer({ text: 'golpeo al enemigo en la cabeza con mi espada', room: {}, participant: {}, inventory: {} });
  ...
  const bufDefend = new CombatBuffer({ text: 'defiendo el golpe con mi escudo', room: {}, participant: {}, inventory: {} });
  ...
  const bufMech = new CombatBuffer({ text: 'apunto al brazo izquierdo del enemigo', room: baseRoom, participant: { bodyParts: {}, name: 'Hero', fulgor: 50, maxFulgor: 50, fatigue: 0, stunned: false }, inventory: { equipped: {} } });
  ...
  const bufNoZone = new CombatBuffer({ text: 'ataco con todo', room: baseRoom, participant: { bodyParts: {}, name: 'Hero', fulgor: 50, maxFulgor: 50, fatigue: 0, stunned: false }, inventory: { equipped: {} } });
  ...
  const room = createMockRoom ? createMockRoom() : {
    id: 'test_room_1',
    participants: [
      { id: 'test-player-1', name: 'TestPlayer', team: 'players', hp: 100, maxHp: 100, fulgor: 50, maxFulgor: 50, fatigue: 0, fuerza: 7, reflejos: 6, velocidad_ataque: 5, precision: 6, velocidad_desplazamiento: 5, dominio_fulgor: 3, resistencia_fisica: 5, resistencia_magica: 3, ko: false, stunned: false, defending: false, defenseMultiplier: 1, bodyParts: { cabeza: 10, cuello: 5, pecho: 20, abdomen: 15, espalda: 15, brazo_izq: 10, brazo_der: 10, pierna_izq: 10, pierna_der: 10, mano_izq: 5, mano_der: 5, pie_izq: 5, pie_der: 5 }, detox: [] },
      { id: 'enemy_goblin_0', name: 'Goblin', team: 'enemies', hp: 30, maxHp: 30, fulgor: 0, maxFulgor: 0, fatigue: 0, fuerza: 4, reflejos: 3, ko: false, bodyParts: { cabeza: 8, pecho: 15, abdomen: 10 } },
    ],
    turnCount: 1, round: 1, location: { zone: 'bosque', region: 'norte', locationId: 'bosque_oscuro' },
  };

  if (!room.participants.find(p => p.id === 'test-player-1')) {
    room.participants.push({ id: 'test-player-1', name: 'TestPlayer', team: 'players', hp: 100, maxHp: 100, fulgor: 50, maxFulgor: 50, fatigue: 0, fuerza: 7, reflejos: 6, ko: false, stunned: false, bodyParts: { cabeza: 10, pecho: 20 } });
  }
  if (!room.participants.find(p => p.id === 'enemy_goblin_0')) {
    room.participants.push({ id: 'enemy_goblin_0', name: 'Goblin', team: 'enemies', hp: 30, maxHp: 30, fatigue: 0, fuerza: 4, reflejos: 3, ko: false, bodyParts: { cabeza: 8, pecho: 15 } });
  }
  ...
  try {
    const pipelineResult = await referee.processRoleplay('miro alrededor en busca de enemigos', room, participant, inventory);
    assert(pipelineResult.success === true || pipelineResult.cartaBlanca === true || pipelineResult.error, 'pipeline returned a valid result for observation');
  ...
  try {
    ...
    const fallbackResult = await referee.fallbackProcess('golpeo al enemigo en la cabeza', room, fallbackParticipant || participant);
    assert(fallbackResult.success !== false || fallbackResult.cartaBlanca !== undefined, 'fallbackProcess returned a result');
