...
async function run() {
  ...
  assert(
    result2b.error && result2b.error.includes("solo funciona contra objetivos debilitados"),
  ...
  const stats3f = combatEngine.getEffectiveStats(createMockParticipant({ precision: 15, reflejos: 10 }), [
    "lluvia_torrencial",
    "fuego_activo",
  ]);
  ...
  assertDeepEqual(
    room4c.activeEffects,
    ["viento_fuerte"],
    "removeExpiredEffects removes fuego_activo from activeEffects",
  );
