...
async function saveInventory(playerId, inv) {
  try {
    ...
    console.error('inventoryService: error guardando', err.message);
  }
}
...
async function addItem(playerId, itemId, quantity = 1) {
  ...
  await saveInventory(playerId, inv);
...
async function damageEquippedItem(playerId, slot, damage = 1) {
  ...
  if (!inv.items.find(i => i.itemId === itemId)) {
    inv.items.push({ itemId, quantity: 0 });
  }
