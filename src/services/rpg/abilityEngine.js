  ...
  if (targetId) {
    ...
    if (ability.conditions && ability.conditions.target_hp_under) {
      ...
      if (ratio > ability.conditions.target_hp_under) {
        return { error: `${ability.name} solo funciona contra objetivos debilitados (<${Math.round(ability.conditions.target_hp_under * 100)}% HP). ${target.name} tiene ${Math.round(ratio * 100)}% HP.` };
      }
    }
  }
...
async function applyEffect(effect, room, participant, targetId, options) {
  switch (effect.type) {
    case 'damage_multiplier': {
      ...
      const aResult = await combatEngine.processAttack(
        room, participant.id, targetId, zone, atkOpts
      );
    ...
    case 'aoe_damage': {
      ...
      if (room.startedVia === 'pvp' && participant.bando) {
        affected = turnManager.getAliveParticipants(room).filter(p => p.id !== participant.id && p.bando !== participant.bando);
      } else {
    ...
    case 'shield': {
      if (!participant.buffs) participant.buffs = [];
      const shieldValue = Math.round((participant[effect.stat || 'dominio_fulgor'] || 1) * (effect.value || 0.40));
      participant.buffs.push({
        stat: 'shield',
        value: shieldValue,
