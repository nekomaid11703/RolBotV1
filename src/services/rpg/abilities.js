const { reduceBuffTimers } = require('./combatEngine');
const ABILITIES_REGISTRY = [
  ...
  },
  {
    ...
    cooldown: 3,
    effects: [
      { type: 'defense_multiplier', value: 2.5 },
    ],
  ...
  },
  {
    ...
    cooldown: 5,
    effects: [
      { type: 'aoe_damage', value: 1.0, stat: 'dominio_fulgor' },
    ],
  ...
  },
  {
    ...
    cooldown: 6,
    effects: [
      { type: 'shield', value: 0.40, duration: 3, stat: 'dominio_fulgor' },
    ],
...
function canUseAbility(participant, abilityId) {
  ...
  if (participant.cooldowns[abilityId] && participant.cooldowns[abilityId] > 0) {
    return { canUse: false, reason: `${ability.name} está en enfriamiento (${participant.cooldowns[abilityId]} turnos restantes).` };
  }

  if (ability.cost.fatigue > 0 && (participant.fatigue || 0) + ability.cost.fatigue > 10) {
    return { canUse: false, reason: `Demasiada fatiga para usar ${ability.name} (tendrías ${(participant.fatigue || 0) + ability.cost.fatigue}/10).` };
  }

  if (ability.cost.fulgor > 0 && (participant.fulgor || 0) < ability.cost.fulgor) {
    return { canUse: false, reason: `Fulgor insuficiente. Necesitas ${ability.cost.fulgor}, tienes ${participant.fulgor || 0}.` };
  }

  for (const [stat, min] of Object.entries(ability.requirements)) {
    if ((participant[stat] || 0) < min) {
      return { canUse: false, reason: `${stat} mínimo ${min} requerido para ${ability.name} (tienes ${participant[stat] || 0}).` };
    }
  }
...
function getAvailableAbilities(participant) {
  return ABILITIES_REGISTRY.filter(a => {
    ...
    return true;
  }).map(a => `- ${a.id} (${a.name}): ${a.description} [${a.cost.fulgor > 0 ? a.cost.fulgor + ' fulgor' : ''}${a.cost.fatigue > 0 ? (a.cost.fulgor > 0 ? ', ' : '') + a.cost.fatigue + ' fatiga' : ''}${a.cooldown > 0 ? ', cd: ' + a.cooldown + ' turnos' : ''}]`);
}

function getActiveBuffsDescription(participant) {
  if (!participant.buffs || participant.buffs.length === 0) return 'Ninguno';
  return participant.buffs.map(b => `${b.stat || 'desconocido'} (${b.value > 0 ? '+' : ''}${b.value}, ${b.duration} turnos restantes)`).join(', ');
}
