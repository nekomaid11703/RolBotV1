...
const PRIORITY_KEYWORDS = {
  critical: ['crítico', 'urgente', 'emergencia', 'crash', 'cuelga', 'bloquea', 'seguridad', 'pérdida de datos', 'data loss', 'no funciona', 'no arranca'],
...
async function createReport({ sock, groupId, userId, userName, description, msg }) {
  ...
  const { intent } = await classifyIntent(description, ['valid_bug', 'feature_request', 'not_a_bug', 'spam']);
  if (intent === 'spam' || intent === 'not_a_bug') {
    throw new Error(`Reporte rechazado: ${intent === 'spam' ? 'parece spam' : 'no parece un bug válido'}`);
  }

  const priority = intent === 'feature_request' ? 'low' : determinePriority(description);
  ...
  const { error } = await supabase
    .from('bot_auth_state')
    .insert({ session_id: SESSION_ID, id, data: report });
  if (error) throw new Error(`Error guardando reporte: ${error.message}`);

  await logSystem('Bug report creado', { id, userId, priority, category: intent });
...
async function resolveReport(id, { summary, commit, resolvedBy }) {
  const existing = await getReport(id);
  if (!existing) throw new Error(`Reporte ${id} no encontrado`);
  const updated = {
    ...existing,
    status: 'resolved',
    resolution: { summary, commit, resolvedBy },
  ...
  await logSystem('Bug report resuelto', { id, summary, commit });
  return updated;
}

async function getReportsByPriority(priorities) {
  const open = await getOpenReports();
  if (!priorities || priorities.length === 0) return open;
  return open.filter(r => priorities.includes(r.priority));
}
...
async function markStale(days = 7) {
  const cutoff = new Date(Date.now() - days * 86400000).toISOString();
  const open = await getOpenReports();
  const stale = open.filter(r => r.timestamp < cutoff);
  for (const r of stale) {
    const updated = { ...r, status: 'stale' };
    await supabase
      .from('bot_auth_state')
      .update({ data: updated })
      .eq('session_id', SESSION_ID)
  ...
  await logSystem('Bugs marcados como stale', { count: stale.length, days });
  return stale.length;
}
