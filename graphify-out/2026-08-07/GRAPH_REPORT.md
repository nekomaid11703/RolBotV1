...
## Corpus Check
- 344 files · ~17,870,488 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3391 nodes · 5721 edges · 172 communities (143 shown, 29 thin omitted)
...
- `deriveWeapon()` --calls--> `getWeaponStats()`  [EXTRACTED]
  scripts/simulate_combat/fighterGenerator.js → src/services/rpg/itemStatService.js
- `deriveArmorPiece()` --calls--> `getArmorStats()`  [EXTRACTED]
  scripts/simulate_combat/fighterGenerator.js → src/services/rpg/itemStatService.js
...
Cohesion: 0.10
Nodes (34): chatCommandTails, checkAdminOnly(), checkAdminPerm(), checkBotAdminOnly(), checkCreatorOnly(), checkEconomyAdmin(), checkGroupOnly(), checkPermission() (+26 more)
...
Cohesion: 0.09
Nodes (47): simulateBattle(), applyReactionFatigue(), { BASE_ATTACK_RANGE, KITE_FATIGUE_MULTIPLIER }, {
  calcFatigueRecovery,
  calcFatigueCost,
  getFatigueLevel,
  getMovementRange,
}, characterShape(), createDurability(), { executeAttack, executeReaction, chooseAiReaction, getEffectiveWeaponRange }, executeHalfTurn() (+39 more)
...
Cohesion: 0.08
Nodes (46): { BLOCK_PREFER_DEF_THRESHOLD }, FAMILIES, MATERIAL_RARITY_ORDER, MATERIAL_RARITY_WEIGHTS, TIER_CAPS, TIER_ORDER, {
  ARMOR_SLOTS,
  COVERAGES,
  NO_WEAPON_CHANCE,
  NO_PIECE_CHANCE,
...
Cohesion: 0.10
Nodes (28): buildDefaultProfile(), buildRegistration(), ensureUserProfile(), { filterExisting }, getOrCreateProfile(), getTopActiveUsers(), METADATA_UPDATERS, normalizeActivity() (+20 more)

### Community 14 - "unwarn.js"
Cohesion: 0.08
Nodes (35): resolveCombatEquipment(), ARMOR_SETS, getSet(), { calculateMovementFatigue }, countSetPieces(), COVERAGE_RULES, getCoverage(), getMovementFatigueWithCoverage() (+27 more)
...
Cohesion: 0.06
Nodes (32): 10. Checklist para crear un ítem válido, 11. Pendiente / en expansión, 1. Propósito del manual, 2.1. El tier NO es una medida estándar, 2.2. DPS parejo entre naturalezas, 2.3. 1 mano vs 2 manos: decisión por arma, no regla, 2.4. Melee vs distancia: solo importa el rango, 2.5. Relación material → arma: mínimos por tipo (+24 more)
...
Cohesion: 0.06
Nodes (50): execute(), execute(), { box }, { calculateMovementFatigue, capFatigue, getMovementRange }, { checkAttackRange }, execute(), { findSessionByCharacter, updateDistance, advanceTurn }, { formatCommandUsage } (+42 more)
...
Cohesion: 0.10
Nodes (34): { box }, { calcFatigueCost, capFatigue }, {
  ...
  buildFatigueBar,
}, { getActiveCharacter, setHp }, { rollFlee, executeAttack, executeReaction }, COMBAT_ACTIONS, REACTION_ACTIONS (+26 more)
...
Cohesion: 0.06
Nodes (56): execute(), { findSessionByUser, removeSession }, { formatCombatDisolved }, execute(), { findSessionByCharacter, findSessionByUser }, { formatCombatStatus }, { getActiveCharacter }, { resolveCharacterEquipment } (+48 more)

### Community 39 - "invite.js"
Cohesion: 0.07
Nodes (47): execute(), { executeGroupAction }, { removeParticipant }, { demoteFromAdmin }, execute(), { executeGroupAction }, execute(), { executeGroupAction } (+39 more)
...
Cohesion: 0.08
Nodes (42): armorLine(), fighterSection(), { generateFighter }, { getSpecialTierMult }, main(), materialLine(), { materialName }, { MATERIALS, getMaterialStats } (+34 more)
...
Cohesion: 0.16
Nodes (18): { getOpenReports, getStats, markStale }, { getOwnerJids }, { logSystem, logError }, midnightReview(), logSystem(), getOpenReports(), getStats(), { logSystem } (+10 more)
...
Cohesion: 0.27
Nodes (9): PHYSICAL_STATS, collectMetrics(), equipmentMetrics(), { getCoverage }, materialRarityOf(), { MATERIALS }, { PHYSICAL_STATS }, trackReaction() (+1 more)
...
Cohesion: 0.06
Nodes (51): execute(), { formatCharacter }, { getActiveCharacter }, { getInventory }, { logError }, { resolveCharacterEquipment }, { composeMessage }, execute() (+43 more)
...
Cohesion: 0.12
Nodes (27): { box }, CATEGORY_DISPLAY, execute(), { getOwnerRecords }, { listAdminsForCategory, listAllCategories, getCategoryLabel }, { box }, execute(), { getOwnerRecords } (+19 more)
...
Cohesion: 0.11
Nodes (21): { box }, { formatCommandUsage }, { formatError }, { getCharacterNames }, { isAdmin }, { setActiveCharacter }, usageMessage, buildFormBody() (+13 more)
...
Cohesion: 0.14
Nodes (20): ARMOR_SLOTS, ARROW, COVERAGES, GENERATED_STATS, IRON_FAMILY, ITEM_POOL, SET_BONUS, STAT_CLAMP (+12 more)
...
Cohesion: 0.09
Nodes (24): { createItemDefinition }, define(), IRON_ITEMS, IRON_STATS, itemCatalog, itemCatalog, registry, createEntity() (+16 more)

### Community 95 - "listCharacters"
Cohesion: 0.14
Nodes (24): { applyFatiguePenalties }, applyMaterialAbsorption(), applyPenalties(), attemptBlock(), attemptDodge(), { BLOCK_PREFER_DEF_THRESHOLD }, calculateDamage(), canReact() (+16 more)
...
Cohesion: 0.12
Nodes (27): { box }, execute(), { formatCommandUsage }, { formatDisplayMention, withMentions }, { formatError }, { getFirstMentionedJid }, { isOwner }, { resolveTargetDisplayName } (+19 more)
...
Cohesion: 0.16
Nodes (15): applyAttackFatigue(), { box }, { calcFatigueCost, capFatigue }, execute(), {
  executeAttack,
  executeReaction,
  chooseAiReaction,
  calculateXpReward,
  ...
  setPendingReaction,
  endSession,
}, { formatActionMenu, formatReactionPrompt, buildFatigueBar }, { formatError } (+7 more)
...
Cohesion: 0.05
Nodes (48): main(), say(), { supabase }, TEMP_PREFIXES, discover(), KNOWN_SCHEMA, { logSystem }, { supabase } (+40 more)

### Community 138 - "composeMessage"
Cohesion: 0.11
Nodes (21): { addParticipant }, { box }, execute(), { box }, execute(), { openGroup }, { box }, { closeGroup } (+13 more)
...
Cohesion: 0.20
Nodes (8): getBalance(), { addMoney, getBalance, transferMoney }, { cache, TTLS }, createContext, { recordGroupActivity }, { recordUserActivity }, { supabase }, todos
...
Nodes (19): { applyFatiguePenalties }, {
  ...
  SIM_HP_MULTIPLIER,
  SIM_STAT_BASE,
}, runSimulation(), STAT_KEYS (+11 more)
...
## Knowledge Gaps
- **2088 isolated node(s):** `husky.sh script`, `$schema`, `src/core/bot.js`, `src/commands/**/*.js`, `src/data/**/*.js` (+2083 more)
...
- **Why does `box()` connect `composeMessage` to `command_handler_error.test.js`, `characterService.js`, `economyAdminHelper.js`, `iron_family.test.js`, `box`, `box`, `resolveTargetDisplayName`, `editar_pj.js`, `set_stelas.js`, `dado.js`, `grupo_cerrar.js`, `invite.js`, `item_add.js`, `createReport`, `bugstatus.js`, `inventoryService.js`, `admin_perm_list.js`, `statusService.js`, `dado.js`, `characterConfig.js`, `items.js`, `clases.js`, `getActiveCharacter`, `listCharacters`, `audit_family_generator.js`, `itemStatService.js`, `command_format.test.js`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `supabase` connect `columnRegistry.js` to `bot.js`, `bugReportService.js`, `crear_pj.js`, `grupo_cerrar.js`, `getActiveCharacter`, `inventoryService.js`, `invite.js`, `getUserProfile`, `Plan: Implementacion de Stats Magicas + Correcciones`, `unwarn.js`, `midnight_review.js`, `LRUCache`, `items.js`, `box`, `durabilityPersistenceService.js`, `listCharacters`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `logError()` connect `inventoryService.js` to `bot.js`, `bugReportService.js`, `createReport`, `loggerService.js`, `renombrar_pj.js`, `economyAdminHelper.js`, `crear_pj.js`, `grupo_cerrar.js`, `actividad.js`, `columnRegistry.js`, `midnight_review.js`, `box`?**
